...
const char* ssid = "smzia";     // Enter SSID here
const char* password = "44445555";  //Enter Password here
...
String message = "";
int Vin1, Vin2;
hw_timer_t *AC_timer = NULL;
void IRAM_ATTR onTimer(){
Vin1          = analogRead(currentAnalogInputPin);
Vin2          = analogRead(calibrationPin);
}
...
/* AC_timer = timerBegin(0, 80, true);
timerAttachInterrupt(AC_timer, &onTimer, true);
timerAlarmWrite(AC_timer, 10000, true);
timerAlarmEnable(AC_timer); //Just Enable */
...
void loop() {
  ...
  String mcurrent     = ("I = " + String(FinalCurrent/ 10, 1) + " A");

  if (MANUAL_START) {
    Serial.print("manual start pressed ");
    Serial.println(MANUAL_START);
    digitalWrite(MOT_RLC1_GPO32, LOW);
    digitalWrite(MOT_RLC2_GPO33, LOW);
    Heltec.display -> drawString(0, 0, "MOTOR ON .....");

  } else {
    digitalWrite(MOT_RLC1_GPO32, HIGH);
    digitalWrite(MOT_RLC2_GPO33, HIGH);
    Heltec.display -> drawString(0, 0, "MOTOR OFF");
  }
  Heltec.display -> drawString(0, 15, String(temperature));
  Heltec.display -> drawString(0, 30, String(humidity));
  Heltec.display -> drawString(0, 45, String(mcurrent));
  Heltec.display -> display();
  Serial2.begin(4800);
  //nitrogen();
  //rms_current();
...
void rms_current()                                                                                                   /*codes to run again and again */
{                                      
      ...
      Serial.println(Vin2);
        ...
        if(currentSampleCount > 4)                                                                        /* after 4000 count or 800 milli seconds (0.8 second), do this following codes*/
          { 
...
// float floatMap(float x, float in_min, float in_max, float out_min, float out_max) {
  // return (x - in_min) * (out_max - out_min) / (in_max - in_min) + out_min;
// }RMSCurrentMean*3.3/4095
            FinalCurrent = floatMap(RMSCurrentMean, 0, 4095, 0, 3.3);
     ...
     /*        Serial.print(" The Current RMS value is: ");
      ...
      Serial.print(" The Current final value is: ");
            Serial.print(FinalCurrent);//,decimalPrecision);
