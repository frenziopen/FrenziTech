...
void OLEDisplayString(String DisplayString, int16_t X, int16_t Y);
...
void setup() {
  ...
  OLEDisplayString("HI MY NAME IS CUBEY", 0, 0);
...
/*  FUNCTION: OLEDisplayString
    PURPOSE: Print string onto AB-02's built-in display, with given coordinates.
    INPUTS: (String) String to display,
            (int) X coordinate of beginning position, 
            (int) Y coordinate of beginning position
    LAST EDIT: 07/11/2023
    FURTHER NOTES: Larger strings will clip off of the display, could use a log 
    buffer to limit the characters printed per string, using one could also minimize
    the effort required in giving correct coordinates
*/
void OLEDisplayString(String DisplayString, int16_t X = 0, int16_t Y = 0)
{
  screen.clear();
  screen.display();
  screen.setFont(ArialMT_Plain_10);
  screen.drawString(X, Y, DisplayString);
  screen.display();
}
